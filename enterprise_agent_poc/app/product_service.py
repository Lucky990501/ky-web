...
class TaskService:
    ...
    async def execute(self, task: dict) -> None:
        ...
        try:
            self._store.set_task(task_id, tenant_id, "running", "generating", f"{agent.name}正在执行")
            ...
            self._store.set_task(task_id, tenant_id, "completed", "completed", "任务已完成", run_id=result.run_id, response=result.text, conversation_id=result.conversation_id)
